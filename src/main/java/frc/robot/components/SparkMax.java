...
public class SparkMax implements EncodingMotor {
    ...
    public SparkMax(int canBusId, boolean inverted) {
        ...
        this.driver = new com.revrobotics.CANSparkMax(this.canBusId, CANSparkMaxLowLevel.MotorType.kBrushed);
        this.encoder = this.driver.getEncoder(com.revrobotics.SparkMaxRelativeEncoder.Type.kQuadrature, 8192);
        if (inverted) {
            this.scalar = -1.0;
        }
    }
